...
BasicGame.Game.prototype = {
  ...
  //At a set interval (usually 60 times per second), this function is called to update the game state
  update: function () {
  ...
  damageEnemy: function(enemy, damage) {
    ...
    if (enemy.alive) {
      ...
      //check the sprite key (eg 'greenEnemy') to see if the sprite is a boss
      if (enemy.key === 'boss') {
  ...
  setupPlayer: function() {
    ...
    //setting the anchors to 0.5 centers the x,y co-ordinates. By default they are set to top left corner
    this.player.anchor.setTo(0.5, 0.5);
    this.player.animations.add('fly', [0,1,2], 20, true);
    //20 refers to speed of animation, true refers to whether or not the animation is looped or just plays once.
    this.player.animations.add('ghost', [3,0,3,1], 20, true);
    this.player.play('fly');
    this.physics.enable(this.player, Phaser.Physics.ARCADE);
    this.player.speed = BasicGame.PLAYER_SPEED;
    this.player.body.collideWorldBounds = true;
    //20 x 20 pixel hitbox, centered a little higher than the center
    this.player.body.setSize(20,20,0,-5);
    this.weaponLevel = 0;
  },

  setupEnemies: function() {
    //creating enemyPool, these enemies just fly at you, pose no real threat
    ...
    this.enemyPool.setAll('outOfBoundsKill', 0.5);
    this.enemyPool.setAll('checkWorldBounds', 0.5);
    ...
    //creating shooterPool, these enemies will shoot at you and fly faster
    ...
    //spawn 5 seconds into the game once it begins
    this.nextShooterAt = this.time.now + Phaser.Timer.SECOND * 5;
    this.shooterDelay = BasicGame.SPAWN_SHOOTER_DELAY;

    //creating Boss pool,, for the Boss Battle!!!
    ...
    //sets the animation for the boss sprite
  ...
  setupBullets: function() {
    //sprite group of bullets used by enemy
    ...
    //sprite group of bullets for user
    ...
    this.bulletPool.setAll('outOfBoundsKill', 0.5);
    this.bulletPool.setAll('checkWorldBounds', 0.5);
  ...
  processDelayedEffects: function() {
    ...
    if ( this.bossApproaching && this.boss.y > 80 ) {
      ...
      //allows bouncing off world bounds
  ...
  spawnEnemies: function() {
    ...
    if (this.nextShooterAt < this.time.now && this.shooterPool.countDead() > 0) {
      ...
      //spawn at random location at the top of page
  ...
  displayEnd: function(win) {
    //cant win and lose at the same time
  ...
  quitGame: function (pointer) {
    //  destroy anything no longer needed.
    //  Stop music, delete sprites.
    this.sea.destroy();
    this.player.destroy();
    this.enemyPool.destroy();
